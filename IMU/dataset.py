...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Add these print statements to check types
    ...
    def load_gt(self, i):
        ...
        gt = pload(self.predata_dir, self.sequences[i] + '_gt.p')
        if 'vs' not in gt:
            print(f"Error: 'vs' key missing in ground truth data for sequence {self.sequences[i]}")
        return gt
...
class TUMVIDataset(BaseDataset):
    ...
    def read_data(self, data_dir):
        ...
        f = os.path.join(self.predata_dir, 'dataset-room1_512_16.p')
        if True and os.path.exists(f):
            return
...
#         'orientation': torch.tensor([1, 0, 0, 0])  # Example placeholder
#     }
