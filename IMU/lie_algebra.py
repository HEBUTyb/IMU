...
class SO3:
    ...
    # @classmethod
    # def rotate_vector(cls, vector, quaternion):
    #     # 将四元数转换为旋转矩阵
    #     rot_matrix = cls.from_quaternion(quaternion)
    #     # 应用旋转矩阵到向量
    #     rotated_vector = torch.matmul(rot_matrix, vector.unsqueeze(-1)).squeeze(-1)
    #     return rotated_vector
    @classmethod
    # def rotate_vector(cls, vector, quaternion):
    #     # 确保vector形状为(batch_size, 3)
    #     if vector.ndim < 2:
    #         vector = vector.unsqueeze(0)  # 将vector调整为(batch_size, 3)
    #     if vector.shape[1] != 3:
    #         raise ValueError("Vector should have a size of 3 at dimension 1.")
    #
    #     # 将四元数转换为旋转矩阵
    #     rot_matrix = cls.from_quaternion(quaternion)
    #     # 应用旋转矩阵到向量，确保向量的维度是(batch_size, 3, 1)
    #     rotated_vector = torch.matmul(rot_matrix, vector.unsqueeze(-1)).squeeze(-1)
    #     return rotated_vector
    def rotate_vector(cls, vector, quaternion):
        # 将四元数转换为旋转矩阵
        rot_matrix = SO3.from_quaternion(quaternion)
        # 确保rot_matrix和vector都在同一设备上
        rot_matrix = rot_matrix.to(vector.device)
        # 应用旋转矩阵到向量
        rotated_vector = torch.matmul(rot_matrix, vector.unsqueeze(-1)).squeeze(-1)
        return rotated_vector
    ...
    @classmethod
    # def quaternion_update(cls, q, omega, dt):
    #     """
    #     Update a quaternion based on angular velocity.
    #
    #     Parameters:
    #     q : torch.Tensor
    #         The quaternion representing the initial orientation, shape (..., 4).
    #     omega : torch.Tensor
    ...
    #     Returns:
    #     torch.Tensor
    #         The updated quaternion, shape (..., 4).
    #     """
    #     # 确保输入张量在正确的设备上
    #     device = q.device
    #     # Convert angular velocity to quaternion form
    #     omega_quat = torch.cat([torch.zeros_like(omega[..., :1]), omega], dim=-1) * dt
    #     omega_quat = omega_quat.unsqueeze(0)
    #     # Compute the quaternion increment using the exponential map
    #     q_delta = cls.qexp(omega_quat)
    #     q_delta = q_delta.to(device)
    #     # Update the quaternion by quaternion multiplication
    #     q_updated = cls.qmul(q, q_delta)
    #     # Normalize the updated quaternion to maintain unit length
    #     q_updated_norm = cls.qnorm(q_updated)
    #
    #     return q_updated_norm
    def quaternion_update_batch(cls, q, omega, dt):
        dt_tensor =torch.tensor([dt], device=q.device).unsqueeze(-1)
        # Assume q has shape [batch_size, 4] and omega has shape [batch_size, 3]
        # Convert omega to quaternion format and scale by dt
        # if omega.device != q.device:
        #     omega = omega.to(q.device)
        # dt_tensor=dt_tensor.expand_as(omega[:, :1])
        # omega_quat = torch.cat([torch.zeros(q.shape[0], 1, device=q.device), omega * dt_tensor], dim=1)
        # 确保omega具有预期的批量维度和3个分量
        if len(omega.shape) == 1:
            omega = omega.view(-1, 3)  # 假设omega是扁平的，需要重新塑形

        # 更新dt_tensor的扩展逻辑以匹配修正后的omega
        dt_tensor = torch.tensor([dt], device=q.device).unsqueeze(-1)
        if omega.device != q.device:
            omega = omega.to(q.device)

        # 使用unsqueeze(0)是为了确保omega[:, :1]能够工作，这里不需要改变
        dt_tensor = dt_tensor.expand_as(omega[:, :1])

        # 之后的操作不变
        omega_quat = torch.cat([torch.zeros(q.shape[0], 1, device=q.device), omega * dt_tensor], dim=1)
        # Calculate the quaternion increment using the exponential map (simplified)
        q_delta = SO3.qexp(omega_quat)  # Ensure qexp handles batched input correctly
        # Update quaternion by quaternion multiplication
        q_updated = SO3.qmul(q, q_delta)  # Ensure qmul handles batched input correctly
        # Normalize the updated quaternion
        q_updated_norm = q_updated / q_updated.norm(dim=1, keepdim=True)
        return q_updated_norm
